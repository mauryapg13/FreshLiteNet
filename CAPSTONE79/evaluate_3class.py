...
from dataset_3class import get_stratified_splits, get_val_transforms, TomatoDataset
...
def evaluate_model(model, dataloader, device, mode, num_classes=10):
    ...
    return acc, mae, qwk, f1, time_per_image_ms, all_targets, all_preds
...
def main():
    ...
    for model_name, mode in models_to_evaluate:
        checkpoint_path = f"checkpoints/{model_name}_{mode}_3class.pth"
        ...
        model = get_model(model_name, mode, num_classes=3)
        ...
        # Evaluate
        acc, mae, qwk, f1, inf_time, all_targets, all_preds = evaluate_model(model, test_loader, device, mode, num_classes=3)
        
        # Plot Confusion Matrix
        cm = confusion_matrix(all_targets, all_preds, labels=[0, 1, 2])
        plt.figure(figsize=(8, 6))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Grade A', 'Grade B', 'Grade C'], yticklabels=['Grade A', 'Grade B', 'Grade C'])
        plt.title(f"{model_name.capitalize()} Confusion Matrix (3-Class)")
        plt.xlabel("Predicted Grade")
        plt.ylabel("True Grade")
        os.makedirs("figures", exist_ok=True)
        plt.savefig(f"figures/cm_{model_name}_3class.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    if results:
        ...
        df.to_csv("results/comparison_metrics_3class.csv", index=False)
        print("Results saved to results/comparison_metrics_3class.csv")
    else:
